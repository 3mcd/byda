...
;( function( window, document ) {
    ...
    var running, // Running flag
        base, // Default base path
        animation = {}, // A hash that specifies animation callbacks for stores.
        localCache, // Contains a reference to a local cache like localStorage.
        cache, // Contains a reference to a cache object to send byda changes.
    ...
    function getBydaElements( dom ) {
        dom = 'string' == typeof dom ? new DOMParser().parseFromString( dom, 'text/html' ) : dom || document;
    ...
    /**
     * Get the localCache or cache instance of a store.
     * @param  {String} name Name of a store
     * @return {String}      Value of a store
     */
    function getCached( name ) {
        return localCache ? localCache[ 'byda-' + name ] :
               cache ? cache[ name ] : '';
    }

    /**
     * Set the localCache and cache instance of a store.
     * @param {String} name  Name of a store
     * @param {String} value Value to set
     */
    function setCached( name, value ) {
        if ( localCache ) localCache[ 'byda-' + name ] = value;
        if ( cache ) cache.byda[ name ] = value;
    }
    ...
    function byda( options, callback ) {
        // Halt if the function wasn't passed options or the running flag is active.
    ...
    Store.prototype.compare = function( store ) {
        if ( 'object' == typeof store && Store == store.constructor ) {
    ...
    Store.prototype.commit = function( done ) {
        ...
        var name = this.name; // Reference the name.

        // Get the cached value if no value was retrieved.
        if ( !value ) value = getCached( name ) || '';
        ...
        if ( 'function' == typeof animation[ name ] ) {
            for ( _i = list.length - 1; _i >= 0; _i-- ) {
                clone = list[ _i ].cloneNode( true ); // Clone the 'animating-out' node.

                clone.innerHTML = value; // Set up the innerHTML content of the 'animating-in' node.

                animation[ name ]( list[ _i ], clone, complete );
            }
        } else {
            complete();
        }
    ...
    byda.init = function( options ) {
        ...
        if (options.animation) animation = options.animation; // Animations object.

        localCache = options.local; // A local cache object such as localStorage.

        cache = options.cache; // A custom cache object.

        if ( cache && !cache.byda ) cache.byda = {}; // Create 'byda' property in a custom cache
